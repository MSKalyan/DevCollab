...
export function Spinner({ className = "h-5 w-5 text-merge", label }) {
  return (
...
export function FullPageLoader({ label = "Loading…" }) {
  return (
    <div className="flex min-h-[60vh] w-full flex-col items-center justify-center gap-3">
      <Loader2 className="h-8 w-8 animate-spin text-merge" />
...
export function SkeletonCard() {
  return (
    <div className="surface flex items-center gap-5 p-5">
      ...
      <Skeleton className="hidden h-24 w-24 rounded-lg sm:block" />
    </div>
  );
}
