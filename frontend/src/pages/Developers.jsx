    ...
    >
      <div className="relative mb-6">
        ...
        <input
          ...
          placeholder="Search by name, bio, or GitHub username"
      ...
      {loading ? (
        ...
        <EmptyState icon={Users} title="No developers found" description="Try a different name, bio, or GitHub username." />
      ) : (
        <>
          <SectionHeader count={developers.length}>Developer list</SectionHeader>
          <div className="grid gap-4 md:grid-cols-2">
            {developers.map((developer) => (
              <Link key={developer.id} to={`/developers/${developer.id}`} className="surface surface-hover block p-5">
                <div className="flex items-center gap-3">
                  <Avatar name={developer.name} className="h-11 w-11 text-base" />
                  <div>
                ...
                <div className="mt-4 flex flex-wrap gap-3 text-sm text-ink-muted">
                  ...
                  {developer.github_username && (
                    <a className="inline-flex items-center gap-1 hover:text-merge" href={`https://github.com/${developer.github_username}`} target="_blank" rel="noreferrer">
                      <Github className="h-4 w-4" />GitHub
                    </a>
                  )}
                  {developer.website && (
                    <a className="inline-flex items-center gap-1 hover:text-merge" href={developer.website} target="_blank" rel="noreferrer">
